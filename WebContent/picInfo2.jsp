...
function remlike(){ 
	$.ajax({ 
		...
		type: "POST", 
		dataType: "json", 
		data: $('#remLike').serialize(), //아이디가 like_form인 곳의 모든 정보를 가져와 파라미터 전송 형태(표준 쿼리형태)로 만들어줌 
		success: function(data){ //ajax통신 성공시 넘어오는 데이터 통째 이름 =data 
			console.log("acttype ->"+data.acttype);
			if(data.acttype == 1){
				$('#removeHeart').attr("src", "img/heart1.png");
				$("#like_result").html(data.likeCnt); //id값이 like_result인 html을 찾아서 data.like값으로 바꿔준다.
...
<%-- function addlike(){
	$.ajax({ 
		...
		type: "POST", 
		dataType: 'json', 
		data: $('#addLike').serialize(), //아이디가 like_form인 곳의 모든 정보를 가져와 파라미터 전송 형태(표준 쿼리형태)로 만들어줌 
		success: function(data){ //ajax통신 성공시 넘어오는 데이터 통째 이름 =data 
			console.log("acttype ->"+data.acttype);
			if(data.acttype == 0){
				$('#addHeart').attr("src", "img/heart1.png");
				$("#like_result").html(data.likeCnt); //id값이 like_result인 html을 찾아서 data.like값으로 바꿔준다.
			}
                	...
                	<div class="mb-4 d-flex flex-wrap" >
                       ...
                       <!-------------------- 좋아요 -------------------->
                       		<c:if test="${id!=null }">
									...
									<form id="remLike" style="display: inline-block;">
										<input type="hidden" name="acttype" value="${acttype}">
										<input type="hidden" name="id" value="${id}">
										<input type="hidden" name="pid" value="${pic.pid}">
										<div class="mr-4 mb-2" style="display: inline-block;">
                            				<span>좋아요 수 : <span class="tm-text-gray-dark" id="like_result">${pic.plike }</span>회</span> 
                        				</div>
									</form>	
